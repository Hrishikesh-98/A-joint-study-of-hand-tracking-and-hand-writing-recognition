...
from utils import load_csv,load_npy, Averager, CTCLabelConverter, getVocab, sortTheArray
...
def train(opt):
    ...
    cp2 = np.load('/net/voxel03/misc/extra/data/hrishikesh/labeled_points_image_1024_train.npy')
    ...
    cp3 = np.load('/net/voxel03/misc/extra/data/hrishikesh/points_new_train.npy')
    cp3 = cp3.reshape((cp2.shape[0],1280,6))
    #cp2 = sortTheArray(cp2, "train")
    #cp2 = cp2[np.argsort(cp2[:,:,0])]
    ...
    #print("3")
    l_train = np.load('/net/voxel03/misc/extra/data/hrishikesh/train_new_zero_labels_lower.npy')
    ...
    cp3_train = DataLoader(cp3, batch_size=opt.batch_size)
    ...
    cp2 = np.load('/net/voxel03/misc/extra/data/hrishikesh/labeled_points_image_1024_val.npy')
    cp2 = cp2.reshape((cp2.shape[0],1024,4)) #/1356
    #cp2 = cp2[np.argsort(cp2[:,:,0])]
    #cp2 = sortTheArray(cp2, "val")
    cp3 = np.load('/net/voxel03/misc/extra/data/hrishikesh/points_new_val.npy')
    cp3 = cp3.reshape((cp2.shape[0],1280,6))
    ...
    for i,data in enumerate(cp3_train):
    ...
    for i,data in enumerate(cp3_val):
    ...
    for epoch in range(opt.num_iter):
        ...
        for i, data in enumerate(cp2_train):
            ...
            points = points_train[i].float().to(device).transpose(2,1)
            ...
            batch_size = image.size(0)
            image = image.transpose(2,1)
            preds,_ = model(image,points,data_hw.float().to(device))
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(cp2_val):
                ...
                image_tensors = data
                data_hw = feats_val[i]
                points = points_val[i].float().to(device).transpose(2,1)
...
if __name__ == '__main__':
    ...
    """ Model Architecture """
    parser.add_argument('--input_size', type=int, default=640, help='the size of the LSTM hidden state')
    parser.add_argument('--hidden_size', type=int, default=512, help='the size of the LSTM hidden state')
    parser.add_argument('--output_size', type=int, default=512, help='number of iterations to train for')
    ...
    if not opt.exp_name:
        opt.exp_name = 'model_hwpoint_original_label_lower_image_labeled_nofc_2xradius'
        # print(opt.exp_name)

    #opt.saved_model = "../../../extra/data/hrishikesh/saved_models/model_hwpoint_original_label_lower_image_labeled/best_model_both_4332.pth"
    #opt.saved_model = "../../../extra/data/hrishikesh/saved_models/model_hwpoint_original_label_lower_image_1024_256vec/best_model_both_2875.pth"
