...
def train(opt):
    ...
    trainannFile = "hwnet/ann/train2_stroke_ann.txt"
    valannFile = "hwnet/ann/val_stroke_ann.txt"
    testannFile = "hwnet/ann/train2_stroke_ann.txt"

    # loading data
    cp1_train = get_data(trainannFile, opt.root_img)[28:]
    l_train = np.load("../../../extra/data/hrishikesh/labeled_mask_train2.npy")[28:] #get_data(trainannFile,opt.root_mask)
    l_train = l_train.reshape((l_train.shape[0],256,256))
    ...
    if opt.val:
        ...
        l_val = np.load("../../../extra/data/hrishikesh/labeled_mask_train2.npy")[28:]
    else:
    ...
    #print(model)
    #return
    opt.saved_model = "../../../extra/data/hrishikesh/saved_models/model_iam_word_stroke_Resnet34_LSTMCell_stroke_Unet_labeled_dice/best_model_MSE.pth"
    count = 0
    if opt.saved_model != '':
        print(f'loading pretrained model from {opt.saved_model}')
        if opt.FT:
    ...
    for epoch in range(opt.num_iter):
        ...
        for i, data in enumerate(cp1_train):
            ...
            cost = criterion(preds, true_masks) + dice_loss(F.softmax(preds, dim=1).float(),
...
if __name__ == '__main__':
    ...
    if not opt.exp_name:
        opt.exp_name = 'model_iam_word_stroke_Resnet34_LSTMCell_stroke_Unet_labeled_aug'
        # print(opt.exp_name)
