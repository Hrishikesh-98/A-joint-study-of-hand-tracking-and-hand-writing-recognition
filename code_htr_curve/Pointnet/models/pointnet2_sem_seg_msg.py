...
class get_model(nn.Module):
    def __init__(self, num_classes, normal_channel):
        ...
        self.fc1 = nn.Linear(163840, 2048)
    ...
    def forward(self, xyz):
        ...
        l0_points = torch.reshape(l0_points, (l0_points.shape[0],l0_points.shape[1]*l0_points.shape[2]))
        #print("l0 ",l0_points.shape)

        x = self.drop1(F.relu(self.bn1(self.fc1(l0_points))))
        #print("x ", x.shape)
        transFeat = self.bn2(self.fc2(x))
        x = self.drop2(F.relu(transFeat))
        #print("x ",x.shape)
        x = self.fc3(x)

        x = F.log_softmax(x, dim=1)
        #print("after log ",x.shape)
        #x = x.permute(0, 2, 1)
        return x, transFeat
