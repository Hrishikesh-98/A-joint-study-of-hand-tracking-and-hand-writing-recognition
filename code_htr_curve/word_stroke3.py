...
def train(opt):
    ...
    cp2 = np.load('../../../extra/data/hrishikesh/keypoints_str_train.npy')[:,8,:2]
    cp2 = cp2.reshape((cp2.shape[0],1*2))
    ...
    cp2 = np.load('../../../extra/data/hrishikesh/keypoints_str_'+type+'.npy')[:,8,:2]
    cp2 = cp2.reshape((cp2.shape[0],1*2))
    ...
    for epoch in range(opt.num_iter):
        ...
        for i, data in enumerate(cp1_train):
            ...
            labels = l_train[i,:].float().to(device)
            #print(labels.shape)
            preds = model(hand_track,points)
            #print(preds.size())
            #_,preds_str = preds.log_softmax(1).max(1)
            #preds = torch.cat((points,preds_str.unsqueeze(0).unsqueeze(2)),dim=2)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(cp1_val):
                ...
                labels = l_val[i,:].float().to(device)
                preds = model(hand_track,points)
                #cost = criterion(preds, labels)
                #print(i,' cost : ', cost )
                #_,preds_str = preds.log_softmax(1).max(1)
                ...
                cost = criterion(preds, labels)
                print(i,' cost : ', cost )
                label_point_cloud = preds.detach().cpu().numpy()
                #print(label_point_cloud.shape)
                cloud = np.append(cloud,label_point_cloud.reshape((label_point_cloud.shape[0],1024*3)),axis=0)
        ...
        if min_val_loss > loss_avg_val.val():
            ...
            np.save("../../../extra/data/hrishikesh/label_point_cloud_transformer_MSE_adam_1_"+type+".npy",cloud[1:,:])
            print(cloud.shape)
            if not opt.val:
                torch.save(
...
if __name__ == '__main__':
    ...
    if not opt.exp_name:
        opt.exp_name = 'model_word_stroke_label_point_cloud'
        # print(opt.exp_name)


    #opt.saved_model = "../../../extra/data/hrishikesh/saved_models/model_word_stroke_label_point_cloud/best_model_MSE.pth"
