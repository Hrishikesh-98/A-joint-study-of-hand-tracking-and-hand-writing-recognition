...
from model import Mdl15 as Mdl
import numpy as np
...
def train(opt):
    ...
    cp2 = np.load('../../../extra/data/hrishikesh/sample_points_image_1024_train.npy')
    cp2 = cp2.reshape((cp2.shape[0],1024,3))
    #cp2 = np.load('/net/voxel03/misc/extra/data/hrishikesh/semantic_rep_new_train.npy')
    #l_vecs = np.load('../../../extra/data/hrishikesh/group_rep_image_train.npy')
    #l_vecs = l_vecs.reshape((l_vecs.shape[0],128,640))
    #l_vecs = torch.tensor(l_vecs)
    #l_points = np.load('../../../extra/data/hrishikesh/sample_points_image_128_train.npy')
    ...
    cp2 = np.load('../../../extra/data/hrishikesh/sample_points_image_1024_val.npy')
    cp2 = cp2.reshape((cp2.shape[0],1024,3)) #/1356
    #cp2 = np.load('/net/voxel03/misc/extra/data/hrishikesh/semantic_rep_new_val.npy')
    #l_vecs = np.load('../../../extra/data/hrishikesh/group_rep_image_val.npy')
    #l_vecs = l_vecs.reshape((l_vecs.shape[0],128,640))
    #l_vecs = torch.tensor(l_vecs)
    #l_points = np.load('../../../extra/data/hrishikesh/sample_points_image_128_val.npy')
    #l_points = l_points.reshape((l_points.shape[0],128,3))
    #l_points = torch.tensor(l_points)
    #cp2 = torch.cat((l_points,l_vecs),dim=2)
    #cp1 = cp1.reshape((cp1.shape[0],128,640))
...
if __name__ == '__main__':
    ...
    if not opt.exp_name:
        opt.exp_name = 'model_hwnet_original_text_128'
        # print(opt.exp_name)


    opt.saved_model = "../../../extra/data/hrishikesh/saved_models/model_hwnet_original_text_128/best_model_new_lstm_0.3.pth"
