...
from utils import reset_graph, load_checkpoint, update_hyperparams, draw, \
    save_seq_data, image_pasting_v3_testing, draw_strokes, drawing_strokes
...
def main_testing(test_image_base_dir, test_dataset, test_image_name,
    ...
    test_image_raw_name = test_image_name.split('.')[-2].split('/')[-1]
    print(test_image_raw_name)
    ...
    for sampling_i in range(sampling_num):
        ...
        # data = np.load('/content/drive/MyDrive/virtual_sketching/outputs/sampling/clean_line_drawings__pretrain_clean_line_drawings/seq_data/2_1.npz')
        # lst = data.files
        ...
        #              save_seq=True, draw_order=draw_order)

        # '''drawing_strokes(data[lst[0]], sampling_dir, test_image_raw_name + '_' + str(sampling_i) + '_pred.png', 
        #              ori_img, test_image_size,
        #              data[lst[1]], data[lst[3]], eval_hps_model.min_width, eval_hps_model.cursor_type,
        #              sample_hps_model.raster_size, sample_hps_model.min_window_size,
        #              sess,
        #              pasting_func=paste_v3_func,
        #              save_seq=True, draw_order=draw_order)'''
...
def main(model_name, test_image_name, sampling_num):
    ...
    sampling_base_dir = '/net/voxel03/misc/extra/data/hrishikesh/npzs_stroke_256_train'
...
if __name__ == '__main__':
    ...
    f = open('../../../extra/data/hrishikesh/hwnet-master/ann/train_stroke_ann.txt','r')
    f1 = open('../labels_test.txt','w')
    labels = []
    lines = f.readlines()

    for i,line in enumerate(lines):
      values = line.split()
      args.model = 'pretrain_clean_line_drawings'
      args.sample = 1
      filename = '../../../extra/data/hrishikesh/stroke_256/'+values[0] #+ '.png'
      args.input = filename
      print(i , " ",filename)
