...
def draw_strokes(data, save_root, save_filename, input_img, image_size, init_cursor, infer_lengths, init_width,
                 ...
                 sess,
                 pasting_func=None,
                 save_seq=True, draw_order=False):
    """
    :param data: (N_strokes, 9): flag, x1, y1, x2, y2, r2, s2
    :return:
    """
    print("drawing stroke")
    ...
    for round_idx in range(len(infer_lengths)):
        ...
        for round_inner_i in range(round_length):
            ...
            print("stroke_idx is : ",stroke_idx)
            ...
            print('img',gt_stroke_img.shape)
            print('large',gt_stroke_img_large.shape)
            controls[stroke_idx][0] = pen_state
            controls[stroke_idx][1] = cursor_pos[0]
            controls[stroke_idx][2] = cursor_pos[1]
            controls[stroke_idx][3] = x1y1[0]
            controls[stroke_idx][4] = x1y1[1]
            controls[stroke_idx][5] = x2y2[0]
            controls[stroke_idx][6] = x2y2[1]
            controls[stroke_idx][7] = width2
            controls[stroke_idx][8] = scaling2
            print(controls[stroke_idx,:])
...
def drawing_strokes(data, save_root, save_filename, input_img, image_size, init_cursor, infer_lengths, init_width,
                 cursor_type, raster_size, min_window_size,
                 sess,
                 pasting_func=None,
                 save_seq=True, draw_order=False):
    ...
    canvas_color_with_moving = np.zeros((image_size, image_size, 3), dtype=np.float32)
    frames = []
    data = np.load('./normal.npy',allow_pickle=True)
    ...
    for round_idx in range(data.shape[0]):
        cursor_pos = data[round_idx][0,1:3]  # (2)
        print('curson_pos ', cursor_pos)

        prev_width = init_width
        prev_scaling = 1.0
        prev_window_size = raster_size  # (1)

        for round_inner_i in range(data[round_idx].shape[0]):
            stroke_idx = round_inner_i

            print("current cursor_pos : ",cursor_pos)
            curr_window_size = prev_scaling * prev_window_size
            curr_window_size = np.maximum(curr_window_size, min_window_size)
            curr_window_size = np.minimum(curr_window_size, image_size)

            pen_state = data[round_idx][stroke_idx, 0]
            stroke_params = data[round_idx][stroke_idx, 3:]  # (8)
            ...
            cursor_pos_large = np.minimum(np.maximum(cursor_pos_large, 0.0), float(image_size - 1))  # (2), large-level
            cursor_pos = cursor_pos_large / float(image_size)
            print('prev cursor pos ', cursor_pos)
            if stroke_idx == data[round_idx].shape[0]-1:
              index = stroke_idx
            else:
              index = stroke_idx+1
            cursor_pos = data[round_idx][index,1:3]
