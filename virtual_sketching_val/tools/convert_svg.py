...
if __name__ == '__main__':
    ...
    f = open('../dataset_test.txt','r')
    lines = f.readlines()
    f.close()

    y = os.listdir('../npzs_test/clean_line_drawings__pretrain_clean_line_drawings/seq_data')

    for i,line in enumerate(lines):
      values = line.split()
      filename = values[0].split('/')[-1]
      if filename+'.npz' in y:
        print(i," ",filename)
        npz_path = '../npzs_test/clean_line_drawings__pretrain_clean_line_drawings/seq_data/' + filename + '.npz'
        args.file = npz_path
