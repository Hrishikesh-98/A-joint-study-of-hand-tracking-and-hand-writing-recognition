...
def make_gif(sess, pasting_func, data, init_cursor, image_size, infer_lengths, init_width,
    ...
    for round_idx in range(len(infer_lengths)):
        ...
        for round_inner_i in range(round_length):
            ...
            #cv2.imwrite('./tools/inter/'+str(stroke_idx)+'_canvas.png',canvas_rgb)
            ...
            canvas_vis_png.save('./tools/inter/'+str(stroke_idx)+'_canvas.png', 'PNG')
    ...
    first_frame.save(save_path, save_all=True, append_images=gif_frames, loop=0, duration=3.0)





def making_gif(sess, pasting_func, data, init_cursor, image_size, infer_lengths, init_width,
             save_base,
             cursor_type='next', min_window_size=32, raster_size=128, add_box=True):
    """
    :param data: (N_strokes, 9): flag, x0, y0, x1, y1, x2, y2, r0, r2
    ...
    for round_idx in range(len(data)):
        print('Making progress', round_idx + 1, '/', len(infer_lengths))
        round_length = len(data[round_idx])

        cursor_pos = data[round_idx][cursor_idx,1:3]  # (2)
        ...
        for round_inner_i in range(round_length):
            ...
            cursor_pos = data[round_idx][stroke_idx,1:3]
